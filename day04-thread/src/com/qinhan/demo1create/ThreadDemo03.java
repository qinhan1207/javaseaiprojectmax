...
public class ThreadDemo03 {
    public static void main(String[] args) {
        ...
        // 4.将Callable接口的对象封装成一个真正的线程任务对象：FutureTask对象
        /**
         * 未来任务对象的作用
         *  a.本质是一个线程任务对象，可以交给Thread线程对象处理
         *  b.可以获取线程执行完毕后的结果
         */
        FutureTask<String> f1 = new FutureTask<>(c1);
        // 5.把FutureTask对象作为参数传给Thread线程对象
        ...
        // 获取线程执行完毕后返回的结果
        try {
            // 如果主线程发现第一个线程还没有执行完毕，回让出CPU，等第一个线程执行完毕后，才会往下执行
            System.out.println(f1.get());
        } catch (Exception e) {
            e.printStackTrace();
        }
        try {
            // 如果主线程发现第二个线程还没有执行完毕，回让出CPU，等第二个线程执行完毕后，才会往下执行
